...
class _LightGCN(nn.Module):
    ...
    def forward(self, sub_graph1, sub_graph2, users, items, neg_items):
        ...
        item_ratings = inner_product(item_embs, neg_item_embs)   # [batch_size]
        sup_logits = sup_pos_ratings - sup_neg_ratings - 2*item_ratings            # [batch_size]
        ...
        return sup_logits, pos_ratings_user,ssl_logits_user,pos_ratings_item,ssl_logits_item,svd_pos_ratings_user,svd_ssl_logits_user,svd_pos_ratings_item,svd_ssl_logits_item
...
class GNNEC(AbstractRecommender):
    def __init__(self, config):
        super(GNNEC, self).__init__(config)
        ...
        # # new
        # svd_u, svd_v, v_mul_s, u_mul_s = self.create_svd_mat()
        # svd_u,s,svd_v = torch.svd_lowrank(adj_matrix, q=self.svd_q)
        # u_mul_s = svd_u @ torch.diag(s)
        # svd_adj = u_mul_s @ svd_v.T
        # del svd_u, s, svd_v, u_mul_s
        # new above
        # new
        # 稀疏矩阵转为稠密矩阵，并计算邻居重要性
        dense_mat = adj_matrix.to_dense()
        # sim = torch.mm(dense_mat, dense_mat.T)
        # sim = torch.mm(dense_mat, sim)
        # sim = torch.mul(sim, dense_mat)
        # dense = sim
        pop = torch.sum(dense_mat, dim = 0) * dense_mat
        dense = pop
        # 归一化，并返回稀疏矩阵
        ...
        svd_adj = torch.sparse.FloatTensor(index.t(), data, torch.Size([self.num_users+self.num_items, self.num_users+self.num_items]))
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for bat_users, bat_pos_items, bat_neg_items in data_iter:
                ...
                sup_logits, pos_ratings_user,tot_ratings_user,pos_ratings_item,tot_ratings_item,svd_pos_ratings_user,svd_tot_ratings_user,svd_pos_ratings_item,svd_tot_ratings_item= self.lightgcn(
                    sub_graph1, sub_graph2, bat_users, bat_pos_items, bat_neg_items)
                ...
                # InfoNCE Loss
                pos_ratings_user = torch.exp(pos_ratings_user / self.ssl_temp)
                tot_ratings_user = torch.sum(torch.exp(tot_ratings_user/self.ssl_temp), dim=1)
                clogits_user = -torch.sum(torch.log(pos_ratings_user/ tot_ratings_user))
                # clogits_item = torch.logsumexp(ssl_logits_item / self.ssl_temp, dim=1)
                pos_ratings_item = torch.exp(pos_ratings_item / self.ssl_temp)
                tot_ratings_item = torch.sum(torch.exp(tot_ratings_item / self.ssl_temp), dim=1)
                clogits_item = -torch.sum(torch.log(pos_ratings_item / tot_ratings_item))
                infonce_loss = clogits_item+clogits_user
                ...
                svd_pos_ratings_user = torch.exp(svd_pos_ratings_user / self.ssl_temp)
                svd_tot_ratings_user = torch.sum(torch.exp(svd_tot_ratings_user / self.ssl_temp),dim=1)
                svdlogits_user = -torch.sum(torch.log(svd_pos_ratings_user / svd_tot_ratings_user))
                svd_pos_ratings_item = torch.exp(svd_pos_ratings_item / self.ssl_temp)
                svd_tot_ratings_item = torch.sum(torch.exp(svd_tot_ratings_item / self.ssl_temp), dim=1)
                svdlogits_item = -torch.sum(torch.log(svd_pos_ratings_item / svd_tot_ratings_item))
                svd_loss = svdlogits_user + svdlogits_item
                # svd_loss = torch.sum(svdlogits_user + svdlogits_item)
