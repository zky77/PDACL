...
class GNNEC_longtail(AbstractRecommender):
    def __init__(self, config):
        super(GNNEC_longtail, self).__init__(config)
        ...
        svd_adj = torch.sparse.FloatTensor(index.t(), data, torch.Size(
    ...
    @timer
    def create_svd_mat(self):
        n_nodes = self.num_users + self.num_items
        users_items = self.dataset.train_data.to_user_item_pairs()
        users_np, items_np = users_items[:, 0], users_items[:, 1]

        ratings = np.ones_like(users_np, dtype=np.float32)
        tmp_adj = sp.csr_matrix((ratings, (users_np, items_np)), shape=(self.num_users, self.num_items))
        ...
        adj = scipy_sparse_mat_to_torch_sparse_tensor(tmp_adj).coalesce().cuda(torch.device(self.device))

        svd_u,s,svd_v = torch.svd_lowrank(adj,q=5)
        u_mul_s = svd_u @ torch.diag(s)
        v_mul_s = svd_v @ torch.diag(s)

        return svd_u, svd_v, u_mul_s, v_mul_s
    ...
    def train_model(self):
        ...
        if self.save_flag:
            ...
            for i in range(len(groupid)-1):
                user_file = self.config.data_dir + self.config["dataset"] + '/longtail_user/%s~%s.npy' % (groupid[i], groupid[i+1])
                print(user_file)
                test_user = np.load(user_file)
                # buf, _ = self.evaluate_model()
                # test_user = [i for i in range(100)]
                buf, _ = self.evaluate_model_longtail(test_user)
                self.logger.info("user_inter_cnt:%s~%s, \t\t%s" % (groupid[i], groupid[i+1], buf))
        else:
    ...
    def evaluate_model_longtail(self, test_users):
        flag = False
        self.lightgcn.eval()
        current_result, buf = self.evaluator.evaluate(self, test_users)
